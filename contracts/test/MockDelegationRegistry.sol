 ...
 */
contract MockDelegationRegistry is IDelegationRegistry, ERC165 {
    ...
    function _computeTokenDelegationHash(
        ...
        return
            keccak256(
                abi.encode(delegate, vault, contract_, tokenId, vaultVersion_, delegateVersion_)
            );
    }
        ...
        returns (IDelegationRegistry.DelegationInfo[] memory info)
    {
        ...
        for (uint256 i = 0; i < potentialDelegationHashesLength; ) {
            ...
            IDelegationRegistry.DelegationInfo memory delegationInfo_ = delegationInfo[
                delegateHash
            ];
            ...
            if (type_ == IDelegationRegistry.DelegationType.ALL) {
                ...
                if (
                    delegateHash ==
                    _computeContractDelegationHash(vault, delegate, delegationInfo_.contract_)
                ) {
                    valid = true;
                }
    ...
    {
        return
            _getDelegatesForLevel(vault, IDelegationRegistry.DelegationType.CONTRACT, contract_, 0);
    }
        ...
        for (uint256 i = 0; i < potentialDelegatesLength; ) {
            ...
            if (delegationInfo_.type_ == delegationType) {
                if (delegationType == IDelegationRegistry.DelegationType.ALL) {
                    ...
                    if (
                        delegationHash == _computeAllDelegationHash(vault, delegationInfo_.delegate)
                    ) {
                    ...
                    if (
                        delegationInfo_.contract_ == contract_ && delegationInfo_.tokenId == tokenId
                    ) {
                        // check delegate version by validating the hash
                        if (
                            delegationHash ==
